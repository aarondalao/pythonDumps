...
vidCap.set(3, 640) # set width by 1280
vidCap.set(4, 480) # set height by 720
...
while True:
    ...
    classIds, confs, boundingBox = net.detect( img, confThreshold=0.60)
    ...
    # FOR VIDEO
    # CREATE AN IF STATEMENT TO CHECK IF THE CLASSID IS EMPTY OR NOT
    ...
    # assign new variables for the loop and use zip() to layout the detected objects in the imageSmall
        for classId, confidence, box in zip(classIds.flatten(), confs.flatten(), boundingBox):
            ...
            # attach class name to the bounding box, set the position of the text, and set the font size and font type
            cv2.putText(img, classNames[classId-1], (box[0] +10, box[1]+ 30), cv2.FONT_HERSHEY_SIMPLEX,0.5,(0,255,0),2)

            # attach the confidence value
            cv2.putText(img, str(confidence), (box[0] +150, box[1]+ 30), cv2.FONT_HERSHEY_SIMPLEX,0.5,(0,255,0),2)
    # shows the image 
